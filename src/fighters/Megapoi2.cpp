...
bool Megapoi2::VerifFighter(Arena arena, int i, int j) { 
    ActionMove* action = new ActionMove(this->getX()+i,this->getY()+j); //Crée une action comme si le bot allait se déplacer dans la case séléctionné
        ...
        for (int i = -1; i < 2; i++) {
            for (int j  = -1; j < 2; j++) {   
                ...
                if(VerifFighter(arena,i,j)){ //Lance la fonction VerifFighter, et fait le if si il return "true"
                    action = new ActionMove(this->getX()+i,this->getY()+j); //Deplace le bot sur une case non utilisé
                }
            }
        }              
    }
